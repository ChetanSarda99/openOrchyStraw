...
orch_tracker_init
[[ "$_ORCH_TRACKER_INITIALIZED" == "true" ]] || fail "T1: not initialized"
[[ "$_ORCH_TRACKER_RECENT" -eq 2 ]] || fail "T1: recent not 2"
[[ "$_ORCH_TRACKER_SUMMARY" -eq 8 ]] || fail "T1: summary not 8"
pass "T1: init with defaults"

# ── Test 2: Init with custom params ──
orch_tracker_init 3 5
[[ "$_ORCH_TRACKER_RECENT" -eq 3 ]] || fail "T2: recent not 3"
[[ "$_ORCH_TRACKER_SUMMARY" -eq 5 ]] || fail "T2: summary not 5"
pass "T2: init with custom params"

# ── Test 3: Init with non-numeric defaults gracefully ──
orch_tracker_init "abc" "xyz"
[[ "$_ORCH_TRACKER_RECENT" -eq 2 ]] || fail "T3: non-numeric recent didn't default to 2"
[[ "$_ORCH_TRACKER_SUMMARY" -eq 8 ]] || fail "T3: non-numeric summary didn't default to 8"
pass "T3: non-numeric params default gracefully"

# ── Test 4: Window fails without init ──
_ORCH_TRACKER_INITIALIZED=false
result=$(orch_tracker_window "/nonexistent" 2>&1) && fail "T4: should fail without init" || true
_ORCH_TRACKER_INITIALIZED=false  # Reset for next test
pass "T4: window fails without init"

# ── Test 5: Window fails with missing file ──
orch_tracker_init
result=$(orch_tracker_window "/nonexistent/file.txt" 2>&1) && fail "T5: should fail with missing file" || true
...
orch_tracker_init 2 8
output=$(orch_tracker_window "$TEST_DIR/small.txt")
...
orch_tracker_init 2 8
output=$(orch_tracker_window "$TEST_DIR/medium.txt")
...
orch_tracker_init 2 8
output=$(orch_tracker_window "$TEST_DIR/large.txt")
...
orch_tracker_init 1 2
output=$(orch_tracker_window "$TEST_DIR/preserved.txt")
...
orch_tracker_init
output=$(orch_tracker_window "$TEST_DIR/preamble.txt")
...
orch_tracker_init
output=$(orch_tracker_window "$TEST_DIR/header.txt")
...
orch_tracker_init 2 8
orch_tracker_window "$TEST_DIR/stats.txt" > /dev/null
stats=$(orch_tracker_stats)
...
orch_tracker_init 2 8
orch_tracker_window "$TEST_DIR/compress.txt" > /dev/null
stats=$(orch_tracker_stats)
...
orch_tracker_init  # Reset state
_ORCH_TRACKER_ORIG_LINES=0
result=$(orch_tracker_stats 2>&1) && {
...
orch_tracker_init 2 8
output=$(orch_tracker_window "$TEST_DIR/single.txt")
...
orch_tracker_init
output=$(orch_tracker_window "$TEST_DIR/empty.txt")
...
orch_tracker_init 3 4
output=$(orch_tracker_window "$TEST_DIR/custom.txt")
...
orch_tracker_init 10 8
output=$(orch_tracker_window "$TEST_DIR/overflow.txt")
...
orch_tracker_init 2 3
output=$(orch_tracker_window "$TEST_DIR/subsections.txt")
...
orch_tracker_init 2 5
output=$(orch_tracker_window "$TEST_DIR/nosummary.txt")
...
orch_tracker_init 2 5
output=$(orch_tracker_window "$TEST_DIR/omit.txt")
...
source "$PROJECT_ROOT/src/core/session-tracker.sh"
[[ "$_ORCH_TRACKER_LOADED" == "1" ]] || fail "T22: double-source guard failed"
...
if [[ -f "$real_tracker" ]]; then
    orch_tracker_init 2 8
    output=$(orch_tracker_window "$real_tracker")
    ...
    wind_lines=$(_orch_tracker_line_count "$output")
...
orch_tracker_init 2 5
output=$(orch_tracker_window "$TEST_DIR/order.txt")
...
orch_tracker_init
output=$(orch_tracker_window "$TEST_DIR/milestone.txt")
...
orch_tracker_init 2 3
orch_tracker_window "$TEST_DIR/reset1.txt" > /dev/null
[[ "$_ORCH_TRACKER_MAX_CYCLE" -eq 9 ]] || fail "T26a: max cycle should be 9"

create_tracker "$TEST_DIR/reset2.txt" 5
orch_tracker_init 1 2
orch_tracker_window "$TEST_DIR/reset2.txt" > /dev/null
[[ "$_ORCH_TRACKER_MAX_CYCLE" -eq 4 ]] || fail "T26b: max cycle should be 4 after re-init"
...
orch_tracker_init 2 1
output=$(orch_tracker_window "$TEST_DIR/largecycle.txt")
...
orch_tracker_init 2 8
output=$(orch_tracker_window "$TEST_DIR/linecount.txt")
...
orch_tracker_init 2 0
output=$(orch_tracker_window "$TEST_DIR/nosummary2.txt")
...
orch_tracker_init 0 5
output=$(orch_tracker_window "$TEST_DIR/norecent.txt")
...
orch_tracker_init
output=$(orch_tracker_window "$TEST_DIR/nopreserved.txt")
...
orch_tracker_init 2 3
output=$(orch_tracker_window "$TEST_DIR/tablerecent.txt")
...
orch_tracker_init 2 8
orch_tracker_window "$TEST_DIR/ranges.txt" > /dev/null
stats=$(orch_tracker_stats)
