...
orch_token_budget_init 100000
orch_budget_allocate "agent-2" 2 > /dev/null  # 50000
orch_token_budget_record "agent-2" 20000
if orch_budget_exceeded "agent-2"; then
...
# Test 9: Total used
orch_token_budget_init 100000
orch_budget_allocate "a" 2 > /dev/null
orch_budget_allocate "b" 2 > /dev/null
orch_token_budget_record "a" 15000
orch_token_budget_record "b" 25000
total=$(orch_budget_total_used)
[[ $total -eq 40000 ]] && pass "total used: 40000" || fail "total used: 40000 (got $total)"

# Test 10: Max tokens calculation
orch_token_budget_init 100000
orch_budget_allocate "agent-1" 2 > /dev/null  # 50000 allocated
max_tok=$(orch_budget_to_max_tokens "agent-1")
# 50000 * 30% = 15000
...
orch_budget_allocate "small" 10 > /dev/null  # 1000 allocated
max_tok=$(orch_budget_to_max_tokens "small")
...
orch_token_budget_init 100000
orch_budget_allocate "lazy" 2 > /dev/null  # 50000
...
orch_token_budget_init 100000
alloc=$(orch_budget_allocate "lazy" 2)
# 50000 * 75% = 37500
...
orch_budget_allocate "test-a" 2 > /dev/null
