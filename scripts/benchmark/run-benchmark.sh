...
_load_instances() {
    ...
    case "$suite" in
        ...
        swebench-lite|swebench)
            # Load from tasks/ directory (JSON files)
            local count=0
...
_run_all() {
    ...
    while IFS= read -r instance_json; do
        ...
        if [[ "$parallel" -gt 0 ]]; then
            # Write instance to temp file for parallel execution
