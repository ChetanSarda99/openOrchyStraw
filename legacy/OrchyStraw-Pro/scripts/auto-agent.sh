...
# ── Pixel Agents emitter (optional — PIXEL_ENABLED=1 to activate) ────
if [ -f "$PROJECT_ROOT/src/pixel/emit-jsonl.sh" ]; then
    source "$PROJECT_ROOT/src/pixel/emit-jsonl.sh"
fi
...
parse_config() {
    ...
    while IFS='|' read -r id prompt ownership interval label model; do
...
run_with_model() {
    local agent_id=$1
    local model="${AGENT_MODELS[$agent_id]:-claude}"

    local agent_timeout=${AGENT_TIMEOUT:-300}  # 5 min default per agent

    case "$model" in
        codex)
            log "[$agent_id] Routing to Codex (GPT-5.4) [${agent_timeout}s timeout]"
            timeout "$agent_timeout" codex exec -m gpt-5.4 --full-auto -
            ;;
        gemini)
            log "[$agent_id] Routing to Gemini (3.1 Pro) [${agent_timeout}s timeout]"
            timeout "$agent_timeout" gemini --model gemini-3.1-pro-preview -p -
            ;;
        claude|*)
            log "[$agent_id] Routing to Claude (Opus 4.6) [${agent_timeout}s timeout]"
...
run_agent() {
    ...
    {
        # Inject shared context before agent's prompt
        ...
        # Cross-cycle history (read-only for workers)
...
detect_rogue_writes() {
    ...
    local PROTECTED_FILES=(
        "scripts/auto-agent.sh"         # The orchestrator itself — re-protected (#77 complete)
    ...
    if [ -n "$protected_violations" ]; then
        ...
        printf '%b' "$protected_violations" | while read -r f; do
    ...
    if [ -n "$rogue_files" ]; then
        ...
        printf '%b' "$rogue_files" | while read -r f; do
            [ -n "$f" ] && log "  ROGUE: $f"
        done
...
run_pm() {
...

    {
        ...
        echo ""
        echo "### Step 0: Read Shared Context + Progress"
        echo "- cat prompts/00-shared-context/context.md — what each agent built, needs, blockers"
        echo "- Read each agent's latest log: prompts/<agent>/logs/<agent>-*.log (most recent)"
        echo "- Use blockers to resequence next cycle's work"
        echo ""
        ...
        --dangerously-skip-permissions \
        --output-format text \
        > "$log_file" 2>&1 || log "[01-PM] WARNING: PM review timed out or errored (exit $?)"
...
case "${1:-help}" in
    orchestrate)
        ...
        while true; do
            ...
            # Check usage limits before starting cycle (all 3 models)
            usage_file="$PROMPTS_DIR/00-shared-context/usage.txt"
            bash "$PROJECT_ROOT/scripts/check-usage.sh" 2>/dev/null
            if [ -f "$usage_file" ]; then
                # Read per-model statuses
                claude_usage=$(grep '^claude=' "$usage_file" 2>/dev/null | cut -d= -f2 || echo 0)
                codex_usage=$(grep '^codex=' "$usage_file" 2>/dev/null | cut -d= -f2 || echo 0)
                gemini_usage=$(grep '^gemini=' "$usage_file" 2>/dev/null | cut -d= -f2 || echo 0)
                overall_usage=$(grep '^overall=' "$usage_file" 2>/dev/null | cut -d= -f2 || echo 0)

                # Pause only if Claude is rate-limited (it's the primary workhorse)
                if [ -n "$claude_usage" ] && [ "$claude_usage" -ge 90 ] 2>/dev/null; then
                    log "PAUSED: Claude at ${claude_usage} (threshold: 90) — codex=${codex_usage} gemini=${gemini_usage}"
                    notify "Paused: Claude at ${claude_usage} — waiting for reset" "warning"
                    sleep 60
                    continue
                fi
                # Warn if any model is stressed but don't pause
                if [ -n "$overall_usage" ] && [ "$overall_usage" -ge 80 ] 2>/dev/null; then
                    log "WARNING: Some models stressed — claude=${claude_usage} codex=${codex_usage} gemini=${gemini_usage}"
                fi
            fi
            ...
            usage_val=$(cat "$usage_file" 2>/dev/null || echo "claude=0\ncodex=0\ngemini=0\noverall=0")
...
CTXEOF
            ...
            # ── Lifecycle: Pre-cycle hooks ──
            type -t orch_signal_init &>/dev/null && orch_signal_init 2>/dev/null || true
            type -t orch_init_project &>/dev/null && orch_init_project "$PROJECT_ROOT" 2>/dev/null || true
            type -t pixel_init &>/dev/null && pixel_init 2>/dev/null || true
            ...
            for id in "${AGENT_IDS[@]}"; do
                ...
                # Lifecycle: check if agent should run (dynamic routing / conditional activation)
                should_run=true
                if type -t orch_should_run_agent &>/dev/null; then
                    orch_should_run_agent "$id" "$CYCLE" "$interval" 2>/dev/null || should_run=false
                else
                    # Fallback to simple interval check
                    [ $((CYCLE % interval)) -eq 0 ] || [ "$CYCLE" -eq 1 ] || should_run=false
                fi

                if [ "$should_run" = true ]; then
                    type -t pixel_agent_start &>/dev/null && pixel_agent_start "$id" "${AGENT_PROMPTS[$id]}" 2>/dev/null || true
                    run_agent "$id" &
                    AGENT_PIDS+=($!)
                    AGENTS_RUN=$((AGENTS_RUN + 1))
                else
                    log "[$id] Skipping (interval=$interval, cycle=$CYCLE)"
                fi
            done

            log "$AGENTS_RUN agents launched on branch $CYCLE_BRANCH"

            # Wait for all and track exit codes + self-healing
            AGENT_FAILURES=0
            pid_idx=0
            for pid in "${AGENT_PIDS[@]}"; do
                agent_for_pid="${AGENT_IDS[$pid_idx]:-unknown}"
                if ! wait "$pid" 2>/dev/null; then
                    AGENT_FAILURES=$((AGENT_FAILURES + 1))
                    # Lifecycle: self-heal on failure
                    type -t orch_self_heal &>/dev/null && orch_self_heal "$agent_for_pid" 2>/dev/null || true
                    type -t pixel_agent_done &>/dev/null && pixel_agent_done "$agent_for_pid" "Failed" 2>/dev/null || true
                else
                    type -t pixel_agent_done &>/dev/null && pixel_agent_done "$agent_for_pid" "Finished" 2>/dev/null || true
                fi
                pid_idx=$((pid_idx + 1))
            done

            # Lifecycle: quality gate after all agents
            type -t orch_quality_gate &>/dev/null && orch_quality_gate "$CYCLE" 2>/dev/null || true
            ...
            # ── Lifecycle: Post-cycle hooks ──
            type -t orch_track_cycle &>/dev/null && orch_track_cycle "$CYCLE" "$COMMITS" "$AGENT_FAILURES" 2>/dev/null || true
            type -t orch_refresh_qmd &>/dev/null && orch_refresh_qmd 2>/dev/null || true

            # ── Step 3.5: Detect rogue writes (outside all ownership) ──
            detect_rogue_writes

            # ── Step 4: PM reviews on the feature branch, then merges to main ──
            backup_prompts
            run_pm "$CYCLE"
            ...
            bash_count=$(find "$PROJECT_ROOT/src/core" "$PROJECT_ROOT/scripts" -name '*.sh' 2>/dev/null | wc -l | tr -d ' ')
            prompt_count=$(find "$PROJECT_ROOT/prompts" -name '*.txt' 2>/dev/null | wc -l | tr -d ' ')
            test_count=$(find "$PROJECT_ROOT/tests/core" -name 'test-*.sh' 2>/dev/null | wc -l | tr -d ' ')
            total=$((bash_count + prompt_count))
...
PEOF
            ...
            for id in "${AGENT_IDS[@]}"; do
                ...
                # Update Date: header timestamp (use awk to avoid sed injection via filenames/values)
                awk -v d="**Date:** ${current_date} — ${current_time}" \
                    '{gsub(/\*\*Date:\*\* .*/, d)}1' "$pf" > "${pf}.tmp" && mv "${pf}.tmp" "$pf" 2>/dev/null

                # Update file counts (patterns vary per prompt) — safe awk replacements
                awk -v r="${backend_src} bash modules + ${test_count} test files = $((backend_src + test_count)) total" \
                    '{gsub(/[0-9]+ bash modules \+ [0-9]+ test files = [0-9]+ total/, r)}1' "$pf" > "${pf}.tmp" && mv "${pf}.tmp" "$pf" 2>/dev/null
                awk -v r="${prompt_count} prompt files" \
                    '{gsub(/[0-9]+ prompt files/, r)}1' "$pf" > "${pf}.tmp" && mv "${pf}.tmp" "$pf" 2>/dev/null
                awk -v r="${doc_count} docs" \
                    '{gsub(/[0-9]+ docs/, r)}1' "$pf" > "${pf}.tmp" && mv "${pf}.tmp" "$pf" 2>/dev/null
                awk -v r="Total: $total source files" \
                    '{gsub(/Total:.*source files/, r)}1' "$pf" > "${pf}.tmp" && mv "${pf}.tmp" "$pf" 2>/dev/null
            done

            git add prompts/ 2>/dev/null
            git diff --cached --quiet 2>/dev/null || git commit -m "chore: auto-update all prompts — cycle $CYCLE ($total files, $doc_count components)" 2>/dev/null
            log "All prompts auto-updated: $total files, $doc_count docs"

            # ── Step 6: Cycle complete ──
            log "CYCLE $CYCLE DONE — $COMMITS commits | ${bash_count} bash + ${prompt_count} prompt files"
