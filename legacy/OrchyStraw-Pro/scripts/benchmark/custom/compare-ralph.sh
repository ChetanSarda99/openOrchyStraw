...
# ── Run one approach across all tasks ────────────────────────────
...
_generate_comparison_report() {
    ...
    local ralph_patch orchy_patch

    ralph_rate="$(jq -r '.resolve_rate' "$ralph_summary")"
    orchy_rate="$(jq -r '.resolve_rate' "$orchy_summary")"
    ralph_time="$(jq -r '.avg_wall_time_seconds' "$ralph_summary")"
    orchy_time="$(jq -r '.avg_wall_time_seconds' "$orchy_summary")"
    ralph_rogue="$(jq -r '.rogue_write_total' "$ralph_summary")"
    orchy_rogue="$(jq -r '.rogue_write_total' "$orchy_summary")"
    ralph_resolved="$(jq -r '.resolved' "$ralph_summary")"
    orchy_resolved="$(jq -r '.resolved' "$orchy_summary")"
    ralph_total="$(jq -r '.total' "$ralph_summary")"
    orchy_total="$(jq -r '.total' "$orchy_summary")"

    # ── Numeric validation for jq output ──
    # All values from jq must be numeric before use in awk expressions
    _validate_numeric() {
        local name="$1" val="$2"
        if [[ ! "$val" =~ ^-?[0-9]*\.?[0-9]+$ ]]; then
    ...
    ralph_rate="$(_validate_numeric ralph_rate "$ralph_rate")"
    orchy_rate="$(_validate_numeric orchy_rate "$orchy_rate")"
    ralph_time="$(_validate_numeric ralph_time "$ralph_time")"
    orchy_time="$(_validate_numeric orchy_time "$orchy_time")"
    ralph_rogue="$(_validate_numeric ralph_rogue "$ralph_rogue")"
    orchy_rogue="$(_validate_numeric orchy_rogue "$orchy_rogue")"
    ralph_resolved="$(_validate_numeric ralph_resolved "$ralph_resolved")"
    orchy_resolved="$(_validate_numeric orchy_resolved "$orchy_resolved")"
    ralph_total="$(_validate_numeric ralph_total "$ralph_total")"
    orchy_total="$(_validate_numeric orchy_total "$orchy_total")"

    # Compute avg rogue writes per task (awk -v prevents shell injection)
    ...
    # Compute deltas (awk -v prevents shell injection)
    ...
    # Compute avg patch match (awk -v prevents shell injection)
    local ralph_patch_sum orchy_patch_sum ralph_patch_avg orchy_patch_avg
    ralph_patch_sum="$(jq -s '[.[].patch_match] | add // 0' "$ralph_results")"
    orchy_patch_sum="$(jq -s '[.[].patch_match] | add // 0' "$orchy_results")"
    ralph_patch_sum="$(_validate_numeric ralph_patch_sum "$ralph_patch_sum")"
    orchy_patch_sum="$(_validate_numeric orchy_patch_sum "$orchy_patch_sum")"
    ...
    # Compute avg files touched (awk -v prevents shell injection)
    local ralph_diff_len orchy_diff_len
    ralph_diff_len="$(jq -s '[.[].diff_length] | add // 0' "$ralph_results")"
    orchy_diff_len="$(jq -s '[.[].diff_length] | add // 0' "$orchy_results")"
    ralph_diff_len="$(_validate_numeric ralph_diff_len "$ralph_diff_len")"
    orchy_diff_len="$(_validate_numeric orchy_diff_len "$orchy_diff_len")"
...
REPORT
    ...
    # Build per-task rows by joining on instance_id
    # Use env vars to pass file paths safely (no shell interpolation in Python)
