...
**Good:** "YOU MAY WRITE TO: frontend/ src/components/ | YOU MUST NOT WRITE TO: backend/ prompts/ scripts/ docs/"
...
| Workers (Backend, Frontend, etc.) | 200-380 | Enough context + tasks without overwhelming |
| Design | 150-250 | Fewer tasks, more standards/patterns |
| QA | 150-250 | Focused on review criteria + verification |

**Target range:** PM 150-300, Workers 170-380, QA 150-250. Trim if prompts exceed these - they tend to bloat over cycles.
...
| Frontend | `/test`, `/review`, `/debug`, `/refactor` |
| Design | `/review` |
...
| `subagent-driven-development` | Dispatch sub-agents per task, two-stage review | Backend, Frontend |
...
| Platform-specific skills | Check plugin marketplace for your framework |
...
Frontend: writing-plans, subagent-driven-development, verification-before-completion,
...
**All agents benefit:** Backend searches API docs, Frontend searches design tokens, QA searches prior reports, PM searches everything.
...
Sections: Usage, Backend Status, Frontend Status, Design Status, QA Findings, Blockers, Notes.
...
- `**Date:**` timestamps
- File counts (source files, test files, components, totals)
...
| Prompt references nonexistent files | "Wire up PaymentService.ts" (doesn't exist) | PM verifies file existence before prompting |
| Two agents own same directory | Frontend and Design both edit `src/components/` | Use exclusions: `src/ !src/components/ui/` vs `src/components/ui/` |
