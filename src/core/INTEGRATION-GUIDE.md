...
## New Modules (Cycle 5) — Integration After v0.1.0

### signal-handler.sh — Graceful Shutdown

Replaces the current `cleanup()` function (lines 84-92) with a proper shutdown sequence:
- Sets `_ORCH_SHUTTING_DOWN` flag so no new agents launch during shutdown
- SIGTERM → wait → SIGKILL pattern for agent processes
- Tracks all agent PIDs via `orch_register_agent_pid`

**Integration:**

1. Add `signal-handler` to the source loop (line 31):
```bash
for mod in bash-version logger error-handler cycle-state agent-timeout dry-run config-validator lock-file signal-handler cycle-tracker; do
```

2. Replace `cleanup()` (lines 84-93) with:
```bash
orch_signal_init 10  # 10s grace period before SIGKILL
```

3. In the agent launch loop (line 674), register PIDs:
```bash
run_agent "$id" &
local pid=$!
AGENT_PIDS+=($pid)
orch_register_agent_pid "$pid"
```

4. Before launching agents, check shutdown flag:
```bash
if orch_is_shutting_down; then
    log "Shutdown in progress — not launching new agents"
    break
fi
```
...
Replaces the `EMPTY_CYCLES` counter (lines 43-44, 692-703) with outcome-aware tracking.
A cycle where agents ran successfully but produced no commits is NOT empty.

**Integration:**

1. At cycle start (after line 584):
```bash
orch_tracker_init "$CYCLE"
```

2. After each agent wait (line 687):
```bash
wait "$pid" 2>/dev/null
if [[ $? -eq 0 ]]; then
    orch_tracker_record "$id" "success"
else
    orch_tracker_record "$id" "fail"
fi
```

3. After commit loop (line 717):
```bash
orch_tracker_set_commits "$COMMITS"
orch_tracker_update_streak
```

4. Replace empty cycle check (lines 695-699):
```bash
if orch_tracker_should_stop "$MAX_EMPTY_CYCLES"; then
    log "STOPPING: $(orch_tracker_empty_streak) consecutive empty cycles"
    log "$(orch_tracker_summary)"
    break
fi
```
